...
This repository is private, so GitHub's private vulnerability reporting is not available here.
Email **support@knowall.ai** with the subject line `Security: hermes-reverie`.

- **Put no vulnerability details in the first message.** Say only that you have a security report
  and ask for an encryption key; we will reply with a PGP public key and a named contact, and you
  send the details encrypted.
- Repository administrators and security managers can instead open a draft security advisory
  under **Security → Advisories**; other collaborators and outside reporters should use email.
